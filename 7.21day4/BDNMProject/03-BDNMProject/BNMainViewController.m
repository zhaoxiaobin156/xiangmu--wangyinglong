...
#import "BNHotShopDetailViewController.h"
...
- (BNMainContentView *)mainContentView
{
    if (!_mainContentView)
    {
        ...
        //热门店铺
        [mainContentView setMainContentViewDidSelectHotShopCallback:^(NSInteger index)
         {
             BNHotShopDetailViewController *detailCtrl = [[BNHotShopDetailViewController alloc] init];
             [self.navigationController pushViewController:detailCtrl animated:YES];
        }];
        
        [mainContentView addRecommendTarget:self sel:@selector(enterDetail:)];
        
#if 0
        //精选抢购
        [mainContentView setMainContentViewDidSelectRecommendCallback:^(NSInteger index) {
...
- (void)enterDetail:(NSNumber *)index
{
    NSLog(@"%@",index);
}
